...
/**
 * Rule which reports, if a defined TimerEvent contains a valid
...
module.exports = (): any => {
  ...
  function validateTimerEventDefinition(timerEventDefinition: ITimerEventDefinition, rootNodeId: string, reporter: BpmnLintReporter): void {
    ...
    if (timerIsDate) {
      ...
      /**
       * Stolen from: https://stackoverflow.com/a/14322189
       */
      ...
      const dateIsInvalid: boolean = !iso8601DateRegex.test(timerEventDefinition.timeDate.body);
  ...
  function check(node: IModdleElement, reporter: BpmnLintReporter): void {
    ...
    if (nodeIsEvent) {
      ...
      const nodeContainsNoEventDefinitions: boolean =
